...
namespace GBSecond {
    ...
    class Task{
    public:
        explicit Task(page_id_t pageId,std::vector<Task> *residueCache,DiskManager *diskManager) noexcept :
        pageId_(pageId),residue_cache_(residueCache),diskManager_(diskManager){}
        ~Task() = default;
